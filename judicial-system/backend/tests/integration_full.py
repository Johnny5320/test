...
for d in test_data:
    ...
    if r.status_code == 201:
        ...
        if r2.json()["items"]:
            created_ids.append(r2.json()["items"][0]["id"])
    else:
...
test("expiring_list 存在且非空", test_reminders_has_expiring_list)
test("expiring_list 字段完整", test_expiring_list_has_correct_fields)
test("expiring_list 包含5天到期人员", test_expiring_list_contains_5d_person)
test("expiring_list 包含15天到期人员", test_expiring_list_contains_15d_person)
test("expiring_list 不含已解除人员", test_expiring_list_excludes_active解除)
test("days_remaining 计算正确", test_expiring_list_days_remaining_correct)
test("level 标记正确(7天)", test_expiring_list_level_correct)
...
def test_minor_then_mental():
    ...
    minor_names = [p["name"] for p in r1.json()["items"]]
    ...
    combined = [p["name"] for p in r2.json()["items"]]
...
def test_mental_then_minor():
    ...
    mental_names = [p["name"] for p in r1.json()["items"]]
    combined = [p["name"] for p in r2.json()["items"]]
...
def test_excel_export():
    r = c.get("/api/persons/export/excel", headers=H)
    assert r.status_code == 200
