...
def test_visit_person_is_deleted_flag(client):
    """1.3 软删人员后，其走访记录的 person_is_deleted 应标记 True，在帮人员为 False"""
    r = client.post("/api/persons", json={"name": "软删人", "id_card": "320102199001010029"})
    assert r.json().get("code") == 0
    pid2 = r.json()["data"]["id"]
    client.post("/api/visits", json={"person_id": 1, "visit_date": "2026-01-01", "visitor": "A", "visit_method": "上门"})
    client.post("/api/visits", json={"person_id": pid2, "visit_date": "2026-01-02", "visitor": "B", "visit_method": "电话"})
    # 软删第二人
    client.delete(f"/api/persons/{pid2}")
    r = client.get("/api/visits?page_size=100")
    items = r.json()["data"]["items"]
    by_person = {v["person_id"]: v for v in items}
    assert by_person[1]["person_is_deleted"] is False
    assert by_person[pid2]["person_is_deleted"] is True
