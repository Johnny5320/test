...
class TestExpiringFilter:
    ...
    def test_filter_30d_includes_15d(self, client):
        """15天后到期 → 30天筛选应包含"""
        _create_person(client, client.headers, "15天到期", "320102197203030029",
    ...
    def test_filter_7d_excludes_15d(self, client):
        """15天后到期 → 7天筛选不应包含"""
        _create_person(client, client.headers, "15天到期B", "320102197304040031",
                       edu_end_date=(date.today() + timedelta(days=15)).isoformat())
    ...
    def test_filter_only_active(self, client):
        """已解除人员即使近期到期也不应出现"""
        _create_person(client, client.headers, "已解除到期", "320102197405050044",
    ...
    def test_filter_no_end_date_excluded(self, client):
        """无截止日期的人员不应出现"""
        _create_person(client, client.headers, "无截止日期", "320102197506060057")
        r = client.get("/api/persons?expiring_within_days=30")
        names = [p["name"] for p in r.json()["data"]["items"]]
    ...
    def test_combined_with_is_minor(self, client):
        """到期筛选 + 未成年筛选叠加"""
        _create_person(client, client.headers, "未成年到期", "32010219760707006X",
                       edu_end_date=(date.today() + timedelta(days=10)).isoformat(), is_minor=True)
        _create_person(client, client.headers, "成年到期", "320102197708080072",
    ...
    def test_combined_with_is_mental(self, client):
        """到期筛选 + 精神疾病筛选叠加"""
        _create_person(client, client.headers, "精神到期", "320102197809090085",
                       edu_end_date=(date.today() + timedelta(days=10)).isoformat(), is_mental=True)
        _create_person(client, client.headers, "正常到期", "32010219791010009X",
...
class TestRemindersList:
    ...
    def test_reminders_has_dismiss_list(self, client):
        """提醒接口应返回 dismiss_list"""
        _create_person(client, client.headers, "提醒测试A", "320102198011110109",
                       edu_end_date=(date.today() + timedelta(days=10)).isoformat())
        r = client.get("/api/reminders")
        assert r.status_code == 200
        data = r.json()["data"]
        assert "dismiss_list" in data
        assert isinstance(data["dismiss_list"], list)
        names = [p["name"] for p in data["dismiss_list"]]
    ...
    def test_dismiss_list_has_required_fields(self, client):
        """dismiss_list 每项应包含 name/risk_level/edu_end_date/days_remaining/level"""
        _create_person(client, client.headers, "字段测试", "320102198112120111",
                       edu_end_date=(date.today() + timedelta(days=5)).isoformat())
        r = client.get("/api/reminders")
        data = r.json()["data"]
        item = next((p for p in data["dismiss_list"] if p["name"] == "字段测试"), None)
    ...
    def test_visit_overdue_list_has_required_fields(self, client):
        ...
        _create_person(client, client.headers, "走访字段测试", "320102198201130127")
        r = client.get("/api/reminders")
        data = r.json()["data"]
        # 如果有超期未走访的人员
