...
class TestExpiringFilter:
    ...
    def test_filter_30d_includes_15d(self, client):
        """15天后到期 → 30天筛选应包含"""
        _create_person(client, client.headers, "15天到期", "320102199001011001",
    ...
    def test_filter_7d_excludes_15d(self, client):
        """15天后到期 → 7天筛选不应包含"""
        _create_person(client, client.headers, "15天到期B", "320102199001011002",
                       edu_end_date=(date.today() + timedelta(days=15)).isoformat())
    ...
    def test_filter_only_active(self, client):
        """已解除人员即使近期到期也不应出现"""
        _create_person(client, client.headers, "已解除到期", "320102199001011003",
    ...
    def test_filter_no_end_date_excluded(self, client):
        """无截止日期的人员不应出现"""
        _create_person(client, client.headers, "无截止日期", "320102199001011004")
        r = client.get("/api/persons?expiring_within_days=30")
        names = [p["name"] for p in r.json()["items"]]
    ...
    def test_combined_with_is_minor(self, client):
        ...
        _create_person(client, client.headers, "成年到期", "320102199001011006",
    ...
    def test_combined_with_is_mental(self, client):
        """到期筛选 + 精神疾病筛选叠加"""
        _create_person(client, client.headers, "精神到期", "320102199001011007",
                       edu_end_date=(date.today() + timedelta(days=10)).isoformat(), is_mental=True)
        _create_person(client, client.headers, "正常到期", "320102199001011008",
...
class TestRemindersList:
    ...
    def test_reminders_has_expiring_list(self, client):
        ...
        _create_person(client, client.headers, "提醒测试A", "320102199001011010",
    ...
    def test_expiring_list_has_required_fields(self, client):
        ...
        _create_person(client, client.headers, "字段测试", "320102199001011011",
    ...
    def test_visit_overdue_list_has_required_fields(self, client):
        ...
        _create_person(client, client.headers, "走访字段测试", "320102199001011012")
        r = client.get("/api/reminders")
        data = r.json()
        # 如果有超期未走访的人员
