...
def test_stats_summary(client):
    ...
    assert data["status_missing"] == 1
    assert data["status_focused"] == 0
...
def test_export_excel(client):
    ...
    resp = client.get("/api/persons/export?format=excel")
    assert resp.status_code == 200
...
def test_export_excel_with_filter(client):
    ...
    resp = client.get("/api/persons/export?format=excel?status=在帮")
    assert resp.status_code == 200
    from openpyxl import load_workbook
    wb = load_workbook(filename=__import__("io").BytesIO(resp.content))
    ws = wb.active
...
def test_import_excel_success(client):
    ...
    assert resp.json()["data"]["total"] == 1
    assert resp.json()["data"]["items"][0]["gender"] == "男"  # 从身份证推算
