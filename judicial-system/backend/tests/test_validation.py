...
def client():
    ...
    with TestClient(app) as c:
        login = c.post("/api/auth/login", json={"real_name": "管理员", "password": "***"})
...
def _vid(seq: int) -> str:
    """生成校验位正确的身份证号（基于 3201021990010 + 4位序号）"""
    W = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2]
    C = "10X98765432"
    b = f"3201021990010{seq:04d}"
...
class TestIdCard:
    ...
    def test_valid_id_card_with_x(self, client):
        """18位，最后一位X — 合法"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "32010219900101007X"})
        assert r.status_code == 200
    ...
    def test_strict_check_digit_rejected(self, client):
        """1.2 严格校验：校验位错误的身份证号必须被拒绝（信封 code=10001）"""
        r = client.post("/api/persons", json={"name": "假号", "id_card": "320113196907177577"})
        assert r.json()["code"] == 10001

    def test_strict_gender_conflict_rejected(self, client):
        """1.2 严格校验：性别与身份证第17位编码不一致必须被拒绝"""
        # 320102197304040031 第17位为奇数→男；传女应冲突
        r = client.post("/api/persons", json={"name": "冲突", "id_card": "320102197304040031", "gender": "女"})
        assert r.json()["code"] == 10001
    ...
    def test_id_card_full_in_list(self, client):
        """列表中不含完整身份证号（隐私脱敏），仅返回 id_card_masked"""
        client.post("/api/persons", json={"name": "张三", "id_card": "320102197809090085"})
        r = client.get("/api/persons")
        item = r.json()["data"]["items"][0]
        assert "id_card" not in item  # 完整号不在列表响应中
        assert "****" in item["id_card_masked"]  # 脱敏号存在

    def test_id_card_full(self, client):
        """详情接口返回完整身份证号，列表接口仅返回脱敏版"""
        client.post("/api/persons", json={"name": "张三", "id_card": "320102198011110109"})
        # 列表：不含完整号
        r = client.get("/api/persons")
        assert "id_card" not in r.json()["data"]["items"][0]
        # 详情：含完整号
        pid = r.json()["data"]["items"][0]["id"]
        r2 = client.get(f"/api/persons/{pid}")
        assert r2.json()["data"]["id_card"]  # 有值即可
...
class TestEnums:
    ...
    def test_status_valid_values(self, client):
        ...
        for i, status in enumerate(["在帮", "已解除", "脱管", "重点关注"]):
            r = client.post("/api/persons", json={"name": f"用户{i}", "id_card": _vid(i), "status": status})
    ...
    def test_risk_level_valid_values(self, client):
        ...
        for i, level in enumerate(["高", "中", "低"]):
            r = client.post("/api/persons", json={"name": f"用户{i}", "id_card": _vid(i), "risk_level": level})
            assert r.status_code == 200
