...
class TestIdCard:
    def test_valid_id_card_18_digits(self, client):
        """18位纯数字 — 合法"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        assert r.status_code == 201

    def test_valid_id_card_with_x(self, client):
        """18位，最后一位X — 合法"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "32010219900101123X"})
        assert r.status_code == 201
    ...
    def test_valid_id_card_with_lowercase_x(self, client):
        ...
        r = client.post("/api/persons", json={"name": "张三", "id_card": "32010219900101123x"})
        assert r.status_code == 201
        assert r.json()["id_card"] == "32010219900101123X"
    ...
    def test_id_card_auto_infer_gender(self, client):
        """从身份证自动推算性别"""
        # 第17位奇数 → 男
        r1 = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        assert r1.json()["gender"] == "男"
        # 第17位偶数 → 女
        r2 = client.post("/api/persons", json={"name": "李四", "id_card": "320102199001011246"})
        assert r2.json()["gender"] == "女"

    def test_id_card_auto_infer_birth_date(self, client):
        """从身份证自动推算出生日期"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        assert r.json()["birth_date"] == "1990-01-01"

    def test_id_card_duplicate(self, client):
        """身份证号重复"""
        client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        r = client.post("/api/persons", json={"name": "李四", "id_card": "320102199001011234"})
        assert r.status_code == 400

    def test_id_card_desensitized_in_list(self, client):
        """列表中身份证号脱敏显示"""
        client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        r = client.get("/api/persons")
        id_card = r.json()["items"][0]["id_card"]
        assert "***" in id_card
        assert id_card.startswith("320102")
        assert id_card.endswith("1234")

    def test_id_card_reveal(self, client):
        """reveal=true 显示完整身份证号"""
        client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        r = client.get("/api/persons?reveal=true")
        assert r.json()["items"][0]["id_card"] == "320102199001011234"
...
class TestPhone:
    def test_valid_phone(self, client):
        """11位手机号 — 合法"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234", "phone": "13800138000"})
        assert r.status_code == 201
    ...
    def test_phone_update(self, client):
        """更新手机号"""
        client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        r = client.put("/api/persons/1", json={"phone": "13900139000"})
...
class TestEnums:
    ...
    def test_status_valid_values(self, client):
        ...
        for i, status in enumerate(["在帮", "已解除", "脱管", "重点关注"]):
            ...
            assert r.status_code == 201, f"status={status} 应该合法"

    def test_risk_level_default(self, client):
        """风险等级默认值 — 低"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234"})
        assert r.json()["risk_level"] == "低"
...
class TestDates:
    ...
    def test_release_date(self, client):
        """释放日期"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011234", "release_date": "2025-06-15"})
        assert r.json()["release_date"] == "2025-06-15"
    ...
    def test_edu_dates(self, client):
        ...
        assert r.json()["edu_start_date"] == "2025-07-01"
        assert r.json()["edu_end_date"] == "2028-06-30"
...
class TestRequired:
    def test_name_required(self, client):
        """姓名必填"""
        r = client.post("/api/persons", json={"id_card": "320102199001011234"})
        assert r.status_code == 422
...
class TestBoundary:
    ...
    def test_name_max_length(self, client):
        """姓名超过20字"""
        r = client.post("/api/persons", json={"name": "这是一个超过二十个字符的姓名测试用例一二三四五六", "id_card": "320102199001011234"})
        assert r.status_code == 422
