...
class TestIdCard:
    def test_valid_id_card_18_digits(self, client):
        """18位纯数字 — 合法"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011232"})
    ...
    def test_id_card_auto_infer_gender(self, client):
        """从身份证自动推算性别"""
        # 第17位奇数 → 男
        r1 = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011232"})
        assert r1.json()["data"]["gender"] == "男"
        # 第17位偶数 → 女
        r2 = client.post("/api/persons", json={"name": "李四", "id_card": "320102199001011240"})
    ...
    def test_id_card_duplicate(self, client):
        ...
        r = client.post("/api/persons", json={"name": "李四", "id_card": "320102199001011232"})
        assert r.status_code == 200
    ...
    def test_id_card_full_in_list(self, client):
        ...
        assert id_card == "320102199001011232"
    ...
    def test_id_card_full(self, client):
        ...
        client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011232"})
        r = client.get("/api/persons")
        assert r.json()["data"]["items"][0]["id_card"] == "320102199001011232"
...
class TestPhone:
    def test_valid_phone(self, client):
        """11位手机号 — 合法"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011232", "phone": "13800138000"})
        assert r.status_code == 200
...
class TestDates:
    ...
    def test_release_date(self, client):
        """释放日期"""
        r = client.post("/api/persons", json={"name": "张三", "id_card": "320102199001011232", "release_date": "2025-06-15"})
...
class TestRequired:
    def test_name_required(self, client):
        """姓名必填"""
        r = client.post("/api/persons", json={"id_card": "320102199001011232"})
        assert r.status_code == 200
...
class TestBoundary:
    ...
    def test_name_max_length(self, client):
        """姓名超过20字"""
        r = client.post("/api/persons", json={"name": "这是一个超过二十个字符的姓名测试用例一二三四五六", "id_card": "320102199001011232"})
        assert r.status_code == 200
