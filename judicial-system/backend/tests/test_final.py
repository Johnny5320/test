...
def test_ocr_without_engine():
    import app.api.ocr as ocr_mod
    r = client.post("/api/ocr/scan", headers=HEADERS, files={"file": ("test.png", b"fake", "image/png")})
    assert r.status_code == 200
    data = r.json()
...
def test_ocr_unsupported_file_type():
    r = client.post("/api/ocr/scan", headers=HEADERS, files={"file": ("test.txt", b"hello", "text/plain")})
    assert r.status_code == 400


def test_ocr_extract_fields_empty():
    from app.api.ocr import extract_fields
    fields = extract_fields([])
    assert fields == {}


def test_ocr_extract_fields_with_id_card():
    from app.api.ocr import extract_fields
    results = [
        ([0, 0, 100, 20], "ID:320102199001011234", 0.99),
        ([0, 0, 100, 20], "phone:13800138000", 0.99),
        ([0, 0, 100, 20], "crime:theft", 0.99),
    ]
    fields = extract_fields(results)
    assert fields.get("id_card") == "320102199001011234"
    assert fields.get("gender") == "female" or "gender" in fields


def test_ocr_extract_fields_male():
    from app.api.ocr import extract_fields
    results = [([0, 0, 100, 20], "身份证号：320102199001011231", 0.99)]
    fields = extract_fields(results)
...
def test_person_import_valid_excel():
    ...
    ws = wb.active
    ws.append(["姓名", "身份证号"])
    ws.append(["导入测试", "110101199003077891"])
    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    r = client.post("/api/persons/import/excel",
...
def test_person_import_bad_enum():
    ...
    ws.append(["枚举测试", "320102199001011666", "非法状态"])
    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    r = client.post("/api/persons/import/excel",
...
def test_person_import_bad_date():
    ...
    ws.append(["日期测试", "320102199001011555", "not-a-date"])
    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    r = client.post("/api/persons/import/excel",
...
def test_person_update_not_found():
    r = client.put("/api/persons/99999", json={"name": "x"}, headers=HEADERS)
    assert r.status_code == 404
...
def test_person_export_excel():
    r = client.get("/api/persons/export/excel", headers=HEADERS)
    assert r.status_code == 200


def test_person_export_with_filters():
    r = client.get("/api/persons/export/excel?status=在帮&risk_level=低", headers=HEADERS)
    assert r.status_code == 200
...
def test_visit_update_not_found():
    r = client.put("/api/visits/99999", json={
...
def test_file_list():
    p = _create_person("文件测试")
    r = client.get(f"/api/files/list/{p['id']}", headers=HEADERS)
    assert r.status_code == 200


def test_file_upload_and_delete():
    p = _create_person("文件上传测试")
    r = client.post(f"/api/files/upload?person_id={p['id']}",
                     files={"file": ("test.jpg", b"fake_jpeg", "image/jpeg")},
                     headers=HEADERS)
    assert r.status_code == 200
    fid = r.json()["id"]

    r = client.delete(f"/api/files/{fid}", headers=HEADERS)
    assert r.status_code == 200


def test_file_delete_not_found():
    r = client.delete("/api/files/99999", headers=HEADERS)
    assert r.status_code == 404


def test_file_upload_person_not_found():
    r = client.post("/api/files/upload?person_id=99999",
                     files={"file": ("test.jpg", b"fake", "image/jpeg")},
                     headers=HEADERS)
    assert r.status_code == 404


def test_file_upload_bad_type():
    p = _create_person("文件类型测试")
    r = client.post(f"/api/files/upload?person_id={p['id']}",
...
def test_invalid_token():
    r = client.get("/api/persons", headers={"Authorization": "Bearer invalid_token_here"})
    assert r.status_code == 401
...
def test_ocr_extract_fields_name_only():
    from app.api.ocr import extract_fields
    results = [([0, 0, 100, 20], "name:John", 0.99)]
    fields = extract_fields(results)
    assert "name" not in fields


def test_ocr_extract_fields_sentence_fan():
    from app.api.ocr import extract_fields
    results = [([0, 0, 100, 20], "犯盗窃罪", 0.99)]
    fields = extract_fields(results)
    assert "original_crime" in fields


def test_ocr_extract_fields_no_dates():
    from app.api.ocr import extract_fields
    results = [([0, 0, 100, 20], "just some text", 0.99)]
    fields = extract_fields(results)
    assert "dates_found" not in fields


def test_ocr_file_too_large():
    big = b"x" * (11 * 1024 * 1024)
    r = client.post("/api/ocr/scan", headers=HEADERS,
...
def test_disabled_user_401():
    from app.core.database import engine
    from sqlmodel import Session
    from app.models.user import User
    from app.core.security import hash_password, create_access_token

    uname = "disabled_api_test"
    with Session(engine) as s:
        existing = s.exec(select(User).where(User.username == uname)).first()
        if existing:
            s.delete(existing)
            s.commit()
        u = User(username=uname, hashed_password=hash_password("test123"),
                 real_name="禁用API测试", role="staff", is_active=False)
        s.add(u)
        s.commit()

    token = create_access_token(data={"sub": uname})
    h = {"Authorization": f"Bearer {token}"}
...
def test_nonexistent_user_token():
    from app.core.security import create_access_token
    token = create_access_token(data={"sub": "nonexistent_user_xyz"})
    h = {"Authorization": f"Bearer {token}"}
    r = client.get("/api/persons", headers=h)
    assert r.status_code == 401
