...
from app.models import User, Person, Visit, EditLog, File, Warning
from app.core.security import hash_password

def calc_check(d17):
    w=[7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2]; cs='10X98765432'
    return cs[sum(int(d17[i])*w[i] for i in range(17))%11]

def gen_id_card(seq):
    y=1960+(seq%50); m=1+(seq%12); d=1+(seq%28)
    d17=f"320102{y}{m:02d}{d:02d}{seq%1000:03d}"[:17]
...
def generate():
    with engine.begin() as conn:
        for t in [Visit,EditLog,File,Warning,Person,User]:
            t.__table__.drop(conn, checkfirst=True)
    print("✅ 数据库已清空")
    with engine.begin() as conn:
    ...
    with Session(engine) as s:
        s.add(User(username="admin",hashed_password=hash_password("admin123"),real_name="系统管理员",role="director",force_change_password=False))
        s.commit()
        print("✅ 管理员: admin / admin123")

        today=date.today(); persons=[]
        for i in range(500):
            prov=random.choice(PROVINCES); city=random.choice(CITIES[prov])
            ...
            rl=es-timedelta(days=random.randint(1,60)); ss=rl-timedelta(days=random.randint(180,3650))
            vi={"高":30,"中":90,"低":180}[rk]
            fs=random.choice(SURNAMES); ic=gen_id_card(i)
            g="男" if int(ic[16])%2==1 else "女"
            bd=datetime.strptime(ic[6:14],"%Y%m%d").date()
            p=Person(name=f"{random.choice(SURNAMES)}{random.choice(GIVEN)}",id_card=ic,gender=g,birth_date=bd,
                household_province=prov,household_city=city,household_district=random.choice(DISTRICTS),
                household_town=random.choice(TOWNS),village=random.choice(VILLAGES),
                ...
                original_crime=random.choice(CRIMES),original_sentence=random.choice(SENTENCES),
                prison_place=random.choice(PRISONS),sentence_start_date=ss,release_date=rl,
                edu_start_date=es,edu_end_date=ee,responsible_person=random.choice(RESPONSIBLE),
                status=st,is_key_target=(rk=="高" or random.random()<0.1),
                category=random.choice(["刑满释放","社区矫正","安置帮教"]),risk_level=rk,
                ...
                has_housing=random.random()>0.1,has_drug_history=random.random()<0.08,
                is_recidivist=random.random()<0.05,has_subsidy=random.random()<0.15,
                is_minor=random.random()<0.03,is_xj=random.random()<0.02,is_mental=random.random()<0.02,
                visit_interval_days=vi,responsible_org=random.choice(ORGS),
                created_at=datetime.now(timezone.utc)-timedelta(days=random.randint(0,365)))
            s.add(p); persons.append(p)
        s.commit()
        for p in persons: s.refresh(p)
        print(f"✅ {len(persons)} 人员记录")

        vc=0
        for p in random.sample(persons,200):
            for _ in range(random.randint(1,5)):
                vd=today-timedelta(days=random.randint(0,730))
                ha=random.random()<0.08
                s.add(Visit(person_id=p.id,visit_date=vd,visitor=random.choice(VISITORS),
                    ...
                    has_abnormal=ha,abnormal_detail=random.choice(A_CONTENTS) if ha else None,
                    quarter=f"{vd.year}-Q{(vd.month-1)//3+1}"))
                vc+=1
                if not p.last_visit_date or vd>p.last_visit_date: p.last_visit_date=vd
        s.commit()

        total=s.exec(select(func.count()).where(col(Person.is_deleted)==False)).one()
        active=s.exec(select(func.count()).where(col(Person.is_deleted)==False,Person.status=="在帮")).one()
        vtotal=s.exec(select(func.count()).select_from(Visit)).one()
        print(f"✅ {vc} 走访记录")
        print(f"\n  人员: {total} | 在帮: {active} | 走访: {vtotal}")
