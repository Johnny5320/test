...
约定：HTTP 按 ErrorCode 映射真实状态码；所有 schema 一律 extra="forbid"。
"""
...
# 分页大小：离线单所、数据量小（<1万），不强制上限，按需拉全量。
# 原重构决议 F2 的 MAX_PAGE_SIZE=500 已按用户要求撤销——卡上限会导致
# "拉全量"类操作（走访/留痕/筛选后全量）在数据增长时触 10001 报错。
# MAX_PAGE_SIZE 仅作默认值参考，不再作为校验上界。
MAX_PAGE_SIZE = 100000  # 软参考值，非强制
