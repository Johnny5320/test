...
from app.schemas.stats import (PrisonItem, PrisonPersonItem, QuarterlyReport,
                               RiskScoreResult, StatsSummary, TrendPoint)
...
def get_person_stats_summary(
        session: Session = Depends(get_session)) -> StatsSummary:
...
@router.get("/prisons")
def list_prisons(session: Session = Depends(get_session)) -> List[PrisonItem]:
    """去重服刑场所及人数"""
...
def get_person_risk_score(person_id: int,
                          session: Session = Depends(get_session)) -> RiskScoreResult:
